#!/usr/bin/env bash
# build.sh — rebuild dex-liquidity-pool.wasm from vendored source.

set -euo pipefail

FIXTURE_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
SRC_DIR="${FIXTURE_DIR}/source"
WASM_NAME="dex-liquidity-pool.wasm"
TARGET="wasm32v1-none"

cd "${SRC_DIR}"
cargo build --release --target "${TARGET}"

BUILT_WASM="target/${TARGET}/release/soroban_liquidity_pool_contract.wasm"
